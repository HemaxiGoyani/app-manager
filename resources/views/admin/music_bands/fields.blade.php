<div class="col-sm-9">
    ...
    <div class="form-group col-sm-12">
        ...
        {!! Form::text('name', null, ['class' => 'form-control', 'placeholder' => 'Name of music band']) !!}
    </div>

    <!-- Applications Field -->
    <div class="form-group col-sm-12">
        {!! Form::label('applications', 'Applications:') !!}
        {!! Form::select('applications[]', [], [], ['class' => 'form-control hasAjaxSelect2', 'style'=>'width:99.9%', 'data-placeholder' => 'Any application for the music band?', 'multiple' => 'multiple', 'id' => 'applications']) !!}
    </div>
...
<div class="sImageOuterContainer col-sm-3">
    <div id="sImageContainer_id" class='sImageContainer mayContainError fastAnimation' tabindex='1'>
        <div class='sImagePlaceholderDiv {{ isset($musicBand) ? 'd-none' : '' }}'>
            <div class='sImagePlaceholderText text-center'>
                Select User Avatar<br/>(Max: 1 MB)
            </div>
        </div>
        <div class='sImagePreview {{ isset($musicBand) ? '' : 'd-none' }}'>
            <img id='sImagePreview_id' class='sImagePreviewImg' alt='Avatar'
                 src='{{ isset($musicBand) ? $musicBand->avatar_url['250'] : route('images_default',['resolution' => '250x250']) }}'
            />
...
<div class="form-group col-sm-12">
    ...
    <a href="{{ route('admin.musicBands.index') }}" class="btn btn-default">Cancel</a>
</div>
@push('stackedScripts')
